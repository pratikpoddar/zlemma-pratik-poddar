...
def getSOhtml2(skill1, skill2):
	...
	filedata = urllib2.urlopen("http://quant.stackexchange.com/questions/tagged/"+skill1+"+"+skill2)
	data = filedata.read()
...
def getSOhtml1(skill):
    try:
        ...
        filedata = urllib2.urlopen("http://quant.stackexchange.com/questions/tagged/"+skill)
        data = filedata.read()
...
def prob_A_given_B(skill1, skill2):
    ...
    try:
	if float(parentinfodict['commonques'][skill2]) > 2:
...
not_to_be_included_skills = ['java', 'programming-languages', 'programming', 'kernel', 'kernel-programming']
